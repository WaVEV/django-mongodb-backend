...
class EmbeddedModelndexMixin:
    def check(self, model, connection):
        # The parent check reports E012 based on top level field resolution only.
        # It cannot correctly evaluate missing fields when sub-fields are used,
        # so these errors are discarded and recomputed.
...
class EmbeddedModelIndex(EmbeddedModelndexMixin, Index):
    ...
    def set_name_with_model(self, model):
        ...
        This method overrides Django's base implementation because the default
        logic relies on ``model._meta.get_field(field_name).column``, which
        does not work for ``EmbeddedModelField``.
        """
